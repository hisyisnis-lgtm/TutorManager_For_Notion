...
describe('authenticated notification proxy', () => {
  ...
  it('ignores client-selected topics and keeps the credential upstream', async () => {
    const fetch = workerFetch(async () => new Response('{"event":"message","message":"fixture","topic":"private-fixture","attachment":{"url":"https://private.invalid/file"}}\n')); vi.stubGlobal('fetch', fetch);
  ...
  it('filters split SSE chunks while preserving Korean notification text', async () => {
    ...
    const fetch = workerFetch(async () => new Response(body)); vi.stubGlobal('fetch', fetch);
  ...
  it('reads configured public topics without an account reservation while preserving full text and hiding credentials', async () => {
    ...
    const fetch = workerFetch(async () => new Response(`${JSON.stringify(raw)}\n`));
    vi.stubGlobal('fetch', fetch);
  ...
  it.each([[302, false], [307, false], [302, true], [307, true]])('rejects HTTP %s (stream=%s), cancels its body, and never follows Location', async (status, stream) => {
    ...
    const response = await teacherNotifications(new Request(`https://audit.invalid/notifications${stream ? '?stream=1' : ''}`),
      { NTFY_TOPIC: 'private-fixture', NTFY_TOKEN: 'synthetic-credential' }, {});
    expect(response.status).toBe(502);
    expect(await response.json()).toEqual({ error: '알림을 불러올 수 없습니다.' });
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(fetch.mock.calls[0][0]).toBe(`https://ntfy.sh/private-fixture/${stream ? 'sse' : 'json?poll=1&since=24h'}`);
    expect(fetch.mock.calls[0][1]).toMatchObject({ redirect: 'manual', headers: { Authorization: 'Bearer synthetic-credential' } });
  ...
  it('still bounds history requests to 15 seconds and hides upstream errors', async () => {
    vi.useFakeTimers();
    const fetch = workerFetch((_url, { signal }) => new Promise((_resolve, reject) => {
