...
describe('private notification proxy', () => {
  ...
  it('ignores client-selected topics and keeps the credential upstream', async () => {
    const fetch = vi.fn()
      .mockResolvedValueOnce(Response.json({ username: 'test', role: 'user', reservations: [{ topic: 'private-fixture', everyone: 'deny-all' }] }))
      .mockResolvedValueOnce(new Response('{"event":"message","message":"fixture","topic":"private-fixture","attachment":{"url":"https://private.invalid/file"}}\n')); vi.stubGlobal('fetch', fetch);
    ...
    expect(fetch.mock.calls[1][0]).toBe('https://ntfy.sh/private-fixture/json?poll=1&since=24h');
    expect(fetch.mock.calls[1][1].headers.Authorization).toBe('Bearer synthetic-credential');
  ...
  it('filters split SSE chunks while preserving Korean notification text', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn()
      .mockResolvedValueOnce(Response.json({ username: 'test', role: 'user', reservations: [{ topic: 'fixture', everyone: 'deny-all' }] }))
  ...
  it('public or unverified topics return 503 before requesting message history', async () => {
    const fetch = vi.fn(async () => Response.json({ username: 'test', role: 'user', reservations: [] }));
    vi.stubGlobal('fetch', fetch);
