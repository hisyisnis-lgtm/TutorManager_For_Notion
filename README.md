...
| 수업 일지 | `318838fa-f2a6-81f1-9b9c-fd379b1026ed` |
...
| 회차부족_감지 | checkbox | GitHub Actions 자동 기입 (직접 수정 금지) |
| 시간 회차 부족 | formula | 회차부족_감지 값을 읽어 "⚠ 시간 회차 부족" 표시 (수정 불가) |
...
> 예정 수업도 잔여 시간 회차에서 차감됨 (취소·보강 제외)

**수업 회차 formula:**
```
if(not empty(prop("수업 일시")) and prop("특이사항") != "🟠 보강" and prop("특이사항") != "🚫 취소", 1, 0)
```

**시간 회차 formula:**
```
if(prop("무료 수업") == 0, 0, if(not empty(prop("수업 일시")) and prop("특이사항") != "🟠 보강" and prop("특이사항") != "🚫 취소", toNumber(prop("수업 시간(분)")) / 60, 0))
```
...
| 속성명 | 타입 | 설명 |
|---|---|---|
| 제목 | title | 자동 생성: "이름 M/D" (create_lesson_logs.mjs) |
| 수업 | relation | → 수업 캘린더 DB (dual_property) |
| 학생 | relation | → 학생 DB (dual_property) |
| 오늘 내용 | rich_text | 이번 수업에서 다룬 내용 |
| 숙제 | rich_text | 내준 숙제 |
| 다음 수업 준비 | rich_text | 다음 수업 계획 |
| 학생 참여도 | select | 😊 좋음 / 😐 보통 / 😞 저조 |
| 메모 | rich_text | 특이사항 |

> 수업 완료 후 수업 캘린더에 "수업 일지" 백링크가 자동 연결됨
> 학생 DB에도 "수업 일지" 백링크 확인 가능
...
| 학생 ↔ 수업 일지 | dual_property | 학생 DB에서는 "수업 일지" |
...
| 수업 캘린더 ↔ 수업 일지 | dual_property | 수업 캘린더 DB에서는 "수업 일지" |
...
### check_session_shortage.mjs 동작 상세

- 전체 학생 조회 → `결제 시간 회차 합계`(paid) 수집
- 전체 수업 조회 (수업 일시 오름차순) → 학생별 세션 목록 구성
- 학생별로: 취소·보강·무료·미입력 수업 제외 → 완료 수업 합산 → 잔여 = paid - 완료
- 예정 수업을 날짜 순으로 처리하며 잔여 차감 → 잔여 < 이번 수업 회차이면 `회차부족_감지=true`
- 2:1 수업: 연결된 학생 중 한 명이라도 회차 부족하면 경고 (OR 집계)
- 완료·취소·보강·무료 수업은 항상 `회차부족_감지=false` 유지
- 변경 필요한 항목만 API 호출 (불필요한 호출 최소화)
