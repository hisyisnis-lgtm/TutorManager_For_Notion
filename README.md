...
| 메모 | rich_text | |
| 등록일 | created_time | |
| 수업 이력 | relation | → 수업 이력 DB (dual_property) |
| 결제 | relation | → 결제 DB (dual_property) |
...
**시간 회차 formula:**
```
if(prop("상태") == "🟢 완료", prop("수업 시간(분)") / 60, 0)
```
...
| 수업 단가 ↔ 할인 이벤트 | dual_property | 단가별 이벤트 확인용 |
...
8. **rollup max vs sum**: 단일 레코드에 연결된 rollup은 sum 대신 max 사용 (결제 DB의 시간당 단가, 적용 할인율 등)
