...
| 충돌 | checkbox | 시간 겹침 충돌 시 자동 체크 (check_conflicts 스크립트) |
...
**시간 회차 formula:**
```
if(prop("상태") == "🟢 완료", toNumber(prop("수업 시간(분)")) / 60, 0)
```
...
| 1인 결제 금액 | formula | 학생 수 곱하기 없는 1인 기준 금액 (학생 DB rollup용) |
...
**결제 금액 formula:**
```
round(prop("시간 회차") * prop("시간당 단가") * (1 - prop("적용 할인율(%)") / 100) * if(prop("수업 유형") == "1:1", 1, 2))
```
- 학생 수: `1:1` → 1명, `2:1` → 2명 곱하기 (단가가 1인 기준이므로)

**미수금 formula:**
```
round(prop("시간 회차") * prop("시간당 단가") * (1 - prop("적용 할인율(%)") / 100) * if(prop("수업 유형") == "1:1", 1, 2)) - prop("실제 결제 금액")
```
> formula에서 다른 formula 참조 불가 → 결제 금액 계산식을 그대로 인라인으로 복사

**1인 결제 금액 formula** (학생 수 제외, 학생 DB rollup용):
```
round(prop("시간 회차") * prop("시간당 단가") * (1 - prop("적용 할인율(%)") / 100))
```

**1인 미수금 formula** (학생 수 제외, 학생 DB rollup용):
```
round(prop("시간 회차") * prop("시간당 단가") * (1 - prop("적용 할인율(%)") / 100)) - round(prop("실제 결제 금액") / if(prop("수업 유형") == "1:1", 1, 2))
```

**결제 상태 formula:**
```
if(
  round(prop("시간 회차") * prop("시간당 단가") * (1 - prop("적용 할인율(%)") / 100) * if(prop("수업 유형") == "1:1", 1, 2)) - prop("실제 결제 금액") == 0,
  "완료",
  if(prop("실제 결제 금액") == 0, "미결제", "미완료")
)
```
...
| 수업 유형 설정 ↔ 할인 이벤트 | dual_property | 단가별 이벤트 확인용 |
...
| `check_conflicts_daemon.mjs` | 충돌 감지 데몬 (5분마다 변경 감지 → 충돌 체크) | 로그인 시 자동 |
| `run_sync.bat` | 작업 스케줄러용 배치 (sync_student_status + check_conflicts) | 작업 스케줄러 |
| `run_daemon.vbs` | 데몬을 창 없이 백그라운드로 실행하는 VBScript 래퍼 | 시작 프로그램 |
