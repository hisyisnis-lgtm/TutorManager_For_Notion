...
async function parse(res, teacherBearer) {
  if (res.status === 401 && teacherBearer) handleTeacherAuthExpiry(teacherBearer);
...
export async function fetchStudentNotices(studentToken) {
  ...
  if (res.status === 401) handleStudentAuthExpiry(studentToken, bearer);
...
export async function fetchNotices() {
  ...
  return parse(await fetchWithTimeout(`${WORKER_URL}/notice`, { headers, cache: 'no-store' }), headers.Authorization?.slice(7));
}
