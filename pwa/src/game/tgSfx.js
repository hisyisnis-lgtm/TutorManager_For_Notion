...
const SYNTH = {
  ...
  // 타이틀(스플래시) 징글 — 5음절(2026-08-09 사용자 지정). 도-미-솔-미-도(옥타브).
  //  네 번째에서 한 번 내려앉았다 마지막에 옥타브로 뛰어 '한 마디'로 들린다(그냥 올라가는 음계와 구분).
  //  마지막 음만 길게 남겨 여운. 잠금해제(unlock)와 헷갈리지 않게 간격을 넓히고 꼬리를 길게 뒀다.
  //  ※ 스플래시 게임 컷(BRAND_CUT_MS 이후)에 재생 — 그 뒤 여유가 1.6초라 꼬리까지 들어간다.
  title: (t, v) => {
    const NOTES = [72, 76, 79, 76, 84];   // C5 E5 G5 E5 C6
    const AT = [0, 0.085, 0.17, 0.255, 0.36]; // 마지막 도약 전에 살짝 숨을 준다
    NOTES.forEach((m, i) => {
      const last = i === NOTES.length - 1;
      mallet(mtof(m), t + AT[i], v * (last ? 1 : 0.85), last ? 1.05 : 0.4);
    });
    mallet(mtof(60), t + 0.36, v * 0.35, 1.15);                      // 옥타브 아래 베이스로 두께
    noiseSweep(t + 0.32, 0.34, v * 0.18, 5000, 12000, 'highpass');   // 반짝 — 아주 옅게
  },
  ...
  shotMiss: (t, v) => { blip(1250, t, 0.08, v, 'square', 720); woodblock(420, t, v * 0.6); }, // 팅: 금속성 하강 + 둔탁한 몸통
