...
import { TG, FONT_TITLE, FONT_BODY, FONT_NUM, TOUCH_OPT } from '../tgTokens.js';
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(26,16,20,0.55)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, ...TOUCH_OPT }}>
      <div className="tg-enter" onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: FONT_TITLE, fontSize: 18, color: TG.INK }}>내 프로필</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontFamily: FONT_BODY, fontWeight: 800, fontSize: 17, color: TG.INK, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{displayName}</span>
              {onEditNickname && (
            ...
            <span style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 12.5, color: TG.SUB }}>{tier.name}</span>
        ...
        {isGuest ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <span style={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 13, color: TG.SUB, textAlign: 'center' }}>로그인하면 기기를 바꿔도 기록이 그대로예요</span>
            <button onClick={() => startSocialLogin('kakao')} className="tg-press" style={{ width: '100%', height: 52, borderRadius: 14, border: 'none', background: '#FEE500', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer', ...TOUCH_OPT }}>
              <KakaoLogo />
              <span style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15, color: 'rgba(0,0,0,0.85)' }}>카카오로 시작하기</span>
            </button>
            <button onClick={() => startSocialLogin('google')} className="tg-press" style={{ width: '100%', height: 52, borderRadius: 14, border: '1px solid #747775', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer', ...TOUCH_OPT }}>
              <GoogleLogo />
              <span style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15, color: '#1f1f1f' }}>Google로 계속하기</span>
            </button>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <span style={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 13, color: TG.SUB, textAlign: 'center' }}>로그인됨 · 기록이 안전하게 저장되고 있어요</span>
            {isMemberUser && onLogout && (
              <button onClick={onLogout} className="tg-press" style={{ width: '100%', height: 48, borderRadius: 14, border: '1.5px solid #ebe5de', background: '#fff', cursor: 'pointer', fontFamily: FONT_BODY, fontWeight: 700, fontSize: 15, color: TG.SUB, ...TOUCH_OPT }}>로그아웃</button>
            )}
          </div>
        )}
        ...
        {userId && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 12, background: '#f7f3ee' }}>
            <span style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 11.5, color: TG.SUB, flexShrink: 0 }}>UID</span>
            <span style={{ flex: 1, minWidth: 0, fontFamily: FONT_NUM, fontSize: 12, color: TG.INK, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{userId}</span>
            <button onClick={copyId} aria-label="UID 복사" className="tg-press"
              ...
              <span style={{ fontFamily: FONT_BODY, fontWeight: 700, fontSize: 11.5, color: copied ? '#36C98D' : TG.SUB }}>{copied ? '복사됨' : '복사'}</span>
            </button>
          </div>
        )}
      </div>
