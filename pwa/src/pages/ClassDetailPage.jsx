...
import { parseLessonLog, isEmpty, fetchLogsByClassIds } from '../api/lessonLogs.js';
...
export default function ClassDetailPage() {
  ...
  const res = useCachedResource(`class:full:v2:${id}`, async () => {
    const parsed = parseClass(await getPage(id));
    // 이 수업의 일지 — 쓴 일지(writtenLogId)와 빈 일지(anyLogId)를 구분해 '작성됨/미작성'을 정확히 낸다.
    const logInfo = (await fetchLogsByClassIds([id]))[id] ?? null;
    return { cls: parsed, prep: await loadPrepSlides(parsed), logInfo };
  });
  const cls = res.data?.cls ?? null;
  const prep = res.data?.prep ?? [];
  const logInfo = res.data?.logInfo ?? null;
    ...
    <>
      ...
      <div className="px-4 pt-4 pb-24">
        <Card>
          ...
          {cls.datetime && new Date(cls.datetime) <= new Date() && !cls.notes?.includes('취소') && (
            <Row label="수업 일지" value={
              logInfo?.writtenLogId
                ? <Link to={`/logs/${logInfo.writtenLogId}`} style={{ color: PRIMARY, fontWeight: 600, textDecoration: 'none' }}>작성됨 · 보기</Link>
                : logInfo?.anyLogId
                  ? <Link to={`/logs/${logInfo.anyLogId}/edit`} style={{ color: PRIMARY, fontWeight: 600, textDecoration: 'none' }}>미작성 · 작성하기</Link>
                  : <span style={{ color: TEXT_TERTIARY }}>미작성</span>
            } />
          )}
