...
import { fetchGameBest, submitGameResult, fetchToneWords } from '../api/gameApi.js';
...
// 게임 페이지 한정 다크 톤 — 디자인 시스템 PRIMARY(#7f0005)는 CTA/액센트로 유지하되
// 배경/카드/텍스트는 게임 몰입감을 위한 다크 와인·플럼 톤으로 재정의 (페이지 한정 예외).
// 참조: Twofold Inc.의 절제된 플랫 컬러 스타일 + Smash Land의 임팩트 타이틀 레이아웃.
const TG = {
  BG: 'radial-gradient(ellipse at top, #3A1024 0%, #2A0A16 45%, #1F0A18 100%)',
  BG_SOLID: '#1F0A18',
  TEXT: '#FFFFFF',
  TEXT_SECONDARY: '#C8B4BD',
  TEXT_TERTIARY: '#8B7080',
  CARD_BG: '#2E1224',
  CARD_BG_RAISED: '#3A1A30',
  CARD_BORDER: 'rgba(255,255,255,0.08)',
  CARD_BORDER_STRONG: 'rgba(255,255,255,0.16)',
  TITLE_ACCENT: '#E8C547', // 머스타드 — "빨리 찾기" 타이틀, 강조 액센트
  HEADER_BG: 'rgba(20, 8, 16, 0.82)',
};
...
export default function ToneGamePage() {
  ...
  // 난이도별 단어 풀 캐시 — Worker에서 fetch한 결과를 메모리에 캐싱.
  // 키: difficulty.id ('easy'|'normal'|'hard'), 값: [{ hanzi, pinyin, tones, meaning }]
  // 게임 시작 시 이 캐시에서 풀을 가져와 셔플 → 라운드 단어 결정.
  ...
  // 단어 풀 사전 로드 — 마운트 시 3개 난이도 모두 백그라운드 fetch.
  // Worker가 Cloudflare 엣지 캐시(1h)로 받쳐주고 있어 일반적으로 <200ms.
  // 실패한 난이도는 wordPoolByDiff에 누락 → startGame에서 감지하여 에러 표시.
  useEffect(() => {
    DIFFICULTIES.forEach((d) => {
      fetchToneWords(d.id)
        .then((words) => {
          if (Array.isArray(words) && words.length > 0) {
            setWordPoolByDiff((prev) => ({ ...prev, [d.id]: words }));
          }
        })
        .catch(() => { /* startGame에서 일괄 처리 */ });
    });
  }, []);
  ...
  const startGame = (difficulty) => {
    ...
    if (!pool || pool.length === 0) {
      // 단어 풀 로드 실패 — 알림 후 시작 화면 복귀 + 재시도 트리거
  ...
  return (
    <div style={{
      ...
      // 게임 몰입감을 위한 다크 와인 배경 — 페이지 한정 예외 (디자인 시스템 베이지 → 다크 톤)
      background: TG.BG,
      color: TG.TEXT,
    }}>
      <ToneGameStyles />
      ...
      <div style={{
        flexShrink: 0,
        backgroundColor: TG.HEADER_BG,
        backdropFilter: 'saturate(180%) blur(20px)',
        WebkitBackdropFilter: 'saturate(180%) blur(20px)',
        borderBottom: `1px solid ${TG.CARD_BORDER}`,
      }}>
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              color: TG.TEXT_SECONDARY, flexShrink: 0,
          ...
          <h1 style={{
            fontSize: 17, fontWeight: 700, color: TG.TEXT, margin: 0, flex: 1,
            textWrap: 'balance',
          }}>
            성조 빨리 찾기
          </h1>
  ...
  return (
    <style>{`
      ...
      @keyframes tg-countdown {
        0%   { width: 100%; background-color: #10B981; }
        50%  { background-color: #10B981; }
        62%  { background-color: #E8C547; }
...
function StartScreen({ onStart }) {
  return (
    ...
    {/* MIDDLE zone — 두 줄 임팩트 타이틀 + 5점 인디케이터 + 설명 (수직 중앙) */}
    <div style={{
      flex: 1, display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center',
      paddingTop: 24, paddingBottom: 32,
    }}>
      {/* 타이틀 영역 — 두 줄 스택, 살짝 좌우 어긋난 정렬 (Smash Land 차용) */}
      <div style={{ position: 'relative', marginBottom: 36, width: '100%' }}>
        {/* 데코 — 성조 마크 1성/4성 (Twofold 스타일에 맞춰 작게, 위 좌우) */}
        <div style={{
          ...
          <span className="tg-deco-float-1" style={{ color: TONES[0].color, display: 'inline-flex', opacity: 0.9 }}>
            <ToneMark tone={1} size={28} />
          </span>
          <span className="tg-deco-float-2" style={{ color: TONES[3].color, display: 'inline-flex', opacity: 0.9 }}>
            <ToneMark tone={4} size={28} />
          </span>
        </div>
        ...
        <div style={{
          ...
          color: TG.TEXT, lineHeight: 0.95,
          letterSpacing: '-0.04em',
          textAlign: 'center',
          // 미세한 좌측 시프트로 stack 느낌 — 큰 차이는 없지만 1번 이미지의 어긋난 정렬 차용
          transform: 'translateX(-8px)',
          textShadow: '0 2px 0 rgba(0,0,0,0.25)',
        }}>
          성조
        </div>
        {/* "빨리 찾기" — 둘째 줄, 머스타드, 약간 우측 시프트 */}
        <div style={{
          fontSize: 56, fontWeight: 800,
          color: TG.TITLE_ACCENT, lineHeight: 1,
          letterSpacing: '-0.04em',
          textAlign: 'center',
          marginTop: 4,
      ...
      <p style={{
        fontSize: 14, color: TG.TEXT_SECONDARY, lineHeight: 1.65,
        margin: '0 0 32px', textAlign: 'center', wordBreak: 'keep-all',
      }}>
        한자만 보고 성조를 순서대로 눌러주세요.<br />
        잘 맞힐수록 시간이 짧아져요.
      </p>

      {/* 5개 성조 인디케이터 — 점 + 라벨 (Twofold 미니멀 스타일) */}
      <div style={{
        ...
        {TONES.map((t) => (
          <div key={t.num} style={{
            ...
            <div style={{
              ...
              boxShadow: `0 0 0 1px rgba(255,255,255,0.08), 0 0 12px ${t.color}40`,
            }} />
            <div style={{
              fontSize: 11, color: TG.TEXT_TERTIARY, fontWeight: 600,
              letterSpacing: '-0.2px',
            }}>
              {t.name}
            </div>
    ...
    <div style={{
      ...
      <div style={{
        marginTop: 14, fontSize: 11, color: TG.TEXT_TERTIARY, fontWeight: 500,
        textAlign: 'center',
      }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ paddingTop: 24, textAlign: 'center' }}>
        <h2 style={{
          fontSize: 24, fontWeight: 700, color: TG.TEXT,
          margin: '0 0 6px', lineHeight: 1.17, letterSpacing: '-0.3px',
        }}>
          난이도 선택
        </h2>
        <p style={{
          fontSize: 14, color: TG.TEXT_SECONDARY, margin: 0,
          lineHeight: 1.65,
        }}>
          원하는 난이도를 골라주세요
        </p>
          ...
          return (
            <button
              ...
              style={{
                background: isSelected
                  ? `linear-gradient(135deg, ${TG.CARD_BG_RAISED} 0%, ${TG.CARD_BG} 100%)`
                  : TG.CARD_BG,
                ...
                // 다크 톤에서는 그림자 대신 glow + border 두께로 선택 표현
                boxShadow: isSelected
                  ? `0 0 0 1px ${TG.TITLE_ACCENT}55, 0 8px 24px rgba(232,197,71,0.10)`
                  : '0 2px 8px rgba(0,0,0,0.20)',
                border: isSelected
                  ? `2px solid ${TG.TITLE_ACCENT}`
                  : `1px solid ${TG.CARD_BORDER}`,
                cursor: 'pointer',
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                {/* 카드 라벨 — 다크 톤에서 머스타드/흰색 대비 */}
                <div style={{
                  fontSize: 20, fontWeight: 700,
                  color: isSelected ? TG.TITLE_ACCENT : TG.TEXT,
                  marginBottom: 4, lineHeight: 1.2, letterSpacing: '-0.2px',
                }}>
                  {d.label}
                </div>
                <div style={{
                  fontSize: 13, fontWeight: 600, color: TG.TEXT_SECONDARY,
                  lineHeight: 1.5,
                }}>
                  {d.desc} · 기본 {baseSec}초
                </div>
              ...
              {best && best.bestScore > 0 && (
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{
                    fontSize: 11, fontWeight: 600, color: TG.TEXT_TERTIARY,
                  }}>
                    최고
                  </div>
                  <div className="tabular-nums" style={{
                    fontSize: 16, fontWeight: 700, color: TG.TEXT,
                    letterSpacing: '-0.2px',
                  }}>
                    {best.bestScore}점
                  </div>
    ...
    <div style={{
      ...
      <div
        ...
        style={{
          ...
          textShadow: '0 4px 24px rgba(232,197,71,0.25), 0 2px 0 rgba(0,0,0,0.3)',
        }}
      >
        {count}
      </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div role="status" aria-label={`${totalWords}문제 중 ${wordIndex + (completed ? 1 : 0)}번째`}>
          <span className="tabular-nums" style={{
            fontSize: 22, fontWeight: 800, color: TG.TEXT,
            letterSpacing: '-0.4px',
          }}>
            {wordIndex + (completed ? 1 : 0)}
          </span>
          <span className="tabular-nums" style={{
            fontSize: 18, fontWeight: 600, color: TG.TEXT_TERTIARY,
            margin: '0 2px 0 4px',
          }}>
            / {totalWords}
          </span>
      ...
      <div style={{
        ...
        >
          ...
          <div style={{
            position: 'relative',
            background: TG.CARD_BG, borderRadius: 16,
            border: `1px solid ${TG.CARD_BORDER}`,
            padding: '40px 24px',
            ...
            {completed && (
              <div className="tg-slide-down" style={{
                ...
                textAlign: 'center', fontSize: 12, color: TG.TEXT_TERTIARY, fontWeight: 500,
              }}>
                {word.meaning}
              </div>
            )}
                  ...
                  <div key={idx} style={{
                    ...
                    <div style={{ height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {tone ? (
                        ...
                        <div className="tg-current-pulse" style={{
                          ...
                          backgroundColor: TG.TITLE_ACCENT,
                          willChange: 'transform, opacity',
                        }} />
                      ) : (
                        <div style={{ width: 24, height: 4, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.12)' }} />
                      )}
                    </div>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        color: isCurrent ? TG.TITLE_ACCENT : TG.TEXT,
                    ...
                    <div style={{ height: 26, display: 'flex', alignItems: 'center' }}>
                      {completed && (
                        <div className="tg-slide-down" style={{
                          ...
                          color: tone?.color || TG.TEXT_TERTIARY,
                        }}>
                          {word.pinyin[idx]}
                        </div>
                      )}
            ...
            <div style={{
              ...
              {!completed ? (
                <span className="tabular-nums" style={{
                  fontSize: 16, color: TG.TEXT_SECONDARY, fontWeight: 600,
                  letterSpacing: '-0.2px',
                }}>
    ...
    <div className="tg-stagger" style={{
      ...
      <div style={{ paddingTop: 20, display: 'flex', justifyContent: 'center' }}>
        <div style={{
          ...
          fontSize: 17, fontWeight: 700, color: TG.TEXT,
          letterSpacing: '-0.2px',
        }}>
          <BookOpenIcon weight="fill" size={17} color={TG.TITLE_ACCENT} />
      ...
      {/* 1. 메시지 — 데코 없이 타이폴로지(크고/머스타드/bounce)로만 강조 */}
      <div style={{ textAlign: 'center' }}>
        <h2
          className="tg-celebrate-pop"
          style={{
            fontSize: 36, fontWeight: 800, color: TG.TITLE_ACCENT,
      ...
      <div style={{
        ...
        {/* 신기록 — 머스타드 pill + 호흡 펄스로 시선 유도 */}
        {isNewBest && (
          <div className="tg-newbest-pulse" style={{
            position: 'absolute', top: 14, right: 14,
            display: 'inline-flex', alignItems: 'center', gap: 4,
            padding: '3px 8px', borderRadius: 999,
            background: TG.TITLE_ACCENT, color: TG.BG_SOLID,
        ...
        <div style={{
          fontSize: 12, fontWeight: 500, color: TG.TEXT_TERTIARY,
          marginBottom: 6,
        }}>
          점수
        </div>
        <div className="tabular-nums" style={{
          fontSize: 52, fontWeight: 800, color: TG.TITLE_ACCENT,
          lineHeight: 1, letterSpacing: '-0.04em',
          textShadow: '0 2px 12px rgba(232,197,71,0.25)',
        }}>
          {animatedScore}
        </div>
        ...
        {hasPreviousBest && (
          <div style={{
            ...
            borderTop: `1px solid ${TG.CARD_BORDER}`,
            fontSize: 12, color: TG.TEXT_SECONDARY, fontWeight: 500,
          }}>
            {isNewBest ? '이전 최고' : '최고 기록'}{' '}
            <span className="tabular-nums" style={{ color: TG.TEXT, fontWeight: 700 }}>
              {previousBest}점
            </span>
          </div>
        )}
      ...
      <div style={{
        width: '100%',
        background: TG.CARD_BG, borderRadius: 12,
        border: `1px solid ${TG.CARD_BORDER}`,
      ...
      <div style={{
        ...
        {/* Secondary 1 — 난이도 선택 (다크 톤 secondary) */}
        <div style={{ width: '100%' }}>
          <button
            type="button"
            ...
            style={{
              ...
              cursor: 'pointer',
              backgroundColor: TG.CARD_BG,
              border: `1px solid ${TG.CARD_BORDER_STRONG}`,
              color: TG.TEXT_SECONDARY,
        ...
        {/* Secondary 2 — 홈으로 (동일 다크 톤 secondary) */}
        <div style={{ width: '100%' }}>
          <button
            type="button"
            onClick={onBack}
...
function StatColumn({ label, value }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, color: TG.TEXT_TERTIARY, fontWeight: 500 }}>
        {label}
      </div>
      <div className="tabular-nums" style={{
        fontSize: 16, fontWeight: 700, color: TG.TEXT,
...
function StatDivider() {
  return <div style={{ width: 1, alignSelf: 'center', height: 28, background: 'rgba(255,255,255,0.10)', flexShrink: 0 }} />;
}
